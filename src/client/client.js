...
class Client {
  ...
  constructor(token, config = BotConfigOpts) {
    if (typeof token !== "string")
      throw new DiscordUserBotsError("Invalid token");
    this.config = {
      ...BotConfigOpts,
    };
    this.token = token;
    ...
    this.check_token().then(async (res) => {
      if (res === true) {
        ...
        throw new DiscordAPIError(
          `Discord rejected token "${token}" (Not valid)`
        );
    });
  }
      ...
      switch (message.t) {
        case null: {
          ...
          if (this.ready_status !== ReadyStates.CONNECTED) {
            if (message.d === null)
              throw new DiscordAPIError("Discord refused a connection.");
            ...
            this.ws.send(
              JSON.stringify(new packets.GateWayOpen(this.token, this.config))
            );
            this.on.gateway();
          } else {
  ...
  close() {
    if (this.ws.readyState !== WebSocket.OPEN)
      throw new DiscordUserBotsError(
        "Cannot close a connection that isn't open"
      );
  ...
  terminate() {
    if (this.ws.readyState !== WebSocket.OPEN)
      throw new DiscordUserBotsError(
        "Cannot terminate a connection that isn't open"
      );
    this.config.autoReconnect = false;
    this.ws.terminate();
    this.ws.removeAllListeners();
  ...
  check_token() {
    return new Promise((resolve) => {
      this.requester
        .fetch_request(`users/@me`, undefined, this.clientData, "GET")
        .then((res) => {
  ...
  async fetch_request(link, options = FetchRequestOpts) {
    ...
    if (typeof link !== "string")
      throw new DiscordUserBotsInternalError("Invalid URL");
  ...
  async fetch_messages(limit, channel_id, before_message_id = false) {
    ...
    if (limit > 100)
      throw new DiscordUserBotsError(
        "Cannot fetch more than 100 messages at a time."
      );
    return await this.fetch_request(
      `channels/${channel_id}/messages?${
        before_message_id === false ? "" : `before=${before_message_id}&`
      }limit=${limit}`,
      {
        method: "GET",
        body: null,
